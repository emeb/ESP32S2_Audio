/*
 * esp_gfx.c - ST7735 graphics using the ESP built-in graphics driver
...
#define EXAMPLE_PIN_NUM_DATA0          34
#define EXAMPLE_PIN_NUM_PCLK           33
#define EXAMPLE_PIN_NUM_CS             37
#define EXAMPLE_PIN_NUM_DC             36
#define EXAMPLE_PIN_NUM_RST            35
...
void example_lvgl_demo_ui(lv_obj_t *scr)
{
	...
	lv_obj_set_pos(arc2, 90, 10);
	lv_obj_remove_style(arc2, NULL, LV_PART_KNOB);
	lv_obj_clear_flag(arc2, LV_OBJ_FLAG_CLICKABLE);
	
	label1 = lv_label_create(lv_scr_act());
	lv_obj_set_parent(label1, arc1);
	lv_label_set_long_mode(label1, LV_LABEL_LONG_WRAP);     /*Break the long lines*/
    lv_label_set_recolor(label1, true);                      /*Enable re-coloring by commands in the text*/
    lv_obj_set_width(label1, 48);  /*Set smaller width to make the lines wrap*/
    ...
    lv_obj_set_width(label2, 48);  /*Set smaller width to make the lines wrap*/
...
esp_err_t esp_gfx_init(void)
{
    ...
    ESP_LOGI(TAG, "Install ST7735/ST7789 panel driver");
	...
	ESP_ERROR_CHECK(esp_lcd_panel_set_gap(panel_handle, 0, 24));
...
void esp_gfx_update(int16_t *v)
{
	...
	char txtbuf[6];
	int32_t value = v[1]/82;
    lv_arc_set_value(arc1, value);
    sprintf(txtbuf, "%3d", value);
	lv_label_set_text(label1, txtbuf);
	value = v[0]/82;
    lv_arc_set_value(arc2, value);
    sprintf(txtbuf, "%3d", value);
	lv_label_set_text(label2, txtbuf);
}
