/*
 * i2s_lowlat.c - Low Latency I2S for realtime synthesis
...
static const char *TAG = "I2S_LowLat";
...
/* uncomment this to enable ISR diagnostics */
#define ISR_DIAGS

#ifdef ISR_DIAGS
#define ISR_FLAG_TX    7
#define ISR_FLAG_RX    8
#define GPIO_OUTPUT_PIN_SEL  (1ULL<<ISR_FLAG_TX) | (1ULL<<ISR_FLAG_RX)
#define FLAG_TX(x) gpio_set_level(ISR_FLAG_TX,(x))
#define FLAG_RX(x) gpio_set_level(ISR_FLAG_RX,(x))
#else
#define FLAG_TX(x)
#define FLAG_RX(x)
#endif
...
/*
 * This block is an overview of APIs in i2s_lowlat.c
...
static bool IRAM_ATTR i2s_dma_rx_callback(gdma_channel_handle_t dma_chan, gdma_event_data_t *event_data, void *user_data)
{
    ...
    if (p_i2s->rx) {
		...
		p_i2s->rx_proc_cb(&(((lldesc_t *)finish_desc)->buf), p_i2s->last_buf_size);
		FLAG_RX(0);
    }
    return 0;
}
...
static bool IRAM_ATTR i2s_dma_tx_callback(gdma_channel_handle_t dma_chan, gdma_event_data_t *event_data, void *user_data)
{
    ...
    if (p_i2s->tx) {
		...
		p_i2s->tx_proc_cb(&(((lldesc_t *)finish_desc)->buf), p_i2s->last_buf_size);
		FLAG_TX(0);
    }
    return 0;
}
...
static esp_err_t i2s_alloc_dma_buffer(i2s_port_t i2s_num, i2s_dma_t *dma_obj)
{
    ...
    uint32_t buf_cnt = 2;	// Only allow double-buffering
...
static esp_err_t i2s_destroy_dma_object(i2s_port_t i2s_num, i2s_dma_t **dma)
{
    ...
    free(*dma);
    *dma = NULL;
    ESP_LOGI(TAG, "DMA object destroyed");
    return ESP_OK;
}
...
err:
    ...
    /* Destroy DMA object if failed to allocate memory */
...
esp_err_t i2s_set_clk(i2s_port_t i2s_num, uint32_t rate, uint32_t bits_cfg, i2s_channel_t ch)
{
	...
	ESP_LOGI(TAG, "last_buf_size = %d", p_i2s[i2s_num]->last_buf_size);
...
static esp_err_t i2s_driver_init(i2s_port_t i2s_num, const i2s_config_t *i2s_config)
{
    ...
    ESP_RETURN_ON_FALSE(!((i2s_config->tx_proc_cb==NULL)&&(i2s_config->mode&I2S_MODE_TX)), ESP_ERR_INVALID_ARG, TAG, "TX enabled and callback is NULL");
    ESP_RETURN_ON_FALSE(!((i2s_config->rx_proc_cb==NULL)&&(i2s_config->mode&I2S_MODE_RX)), ESP_ERR_INVALID_ARG, TAG, "RX enabled and callback is NULL");
	...
	p_i2s[i2s_num]->tx_proc_cb = i2s_config->tx_proc_cb;
    p_i2s[i2s_num]->rx_proc_cb = i2s_config->rx_proc_cb;
